...
-- Create table Stocks(stock_name varchar(20), operation varchar(5), operation_day int, price int);
...
Select stock_name, sum(case when operation = 'Sell' then price end)-sum(case when operation = 'Buy' then price end) capital_gain_loss
from Stocks 
group by stock_name;
